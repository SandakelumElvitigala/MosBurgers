...
Promise.all([
    fetch('../json/pasta.json').then(response => response.json()),
...
.then(([subData, foodIdData]) => {
    ...
    if (!localStorage.getItem('pasta')) {
        localStorage.setItem('pasta', JSON.stringify(subData));
    }
...
function displayData4() {
    const container = document.getElementById('data-container4');
    if (!container) {
        console.error('Container not found!');
        return;
    }

    const items = JSON.parse(localStorage.getItem('pasta'));
