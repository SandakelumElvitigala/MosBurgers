...
Promise.all([
    fetch('../json/sub.json').then(response => response.json()),
    fetch('../json/foodId.json').then(response => response.json())
])
.then(([subData, foodIdData]) => {
    console.log('Fetched sub data:', subData);
    console.log('Fetched food ID data:', foodIdData);
    if (!localStorage.getItem('submarines')) {
        localStorage.setItem('submarines', JSON.stringify(subData));
    }
    localStorage.setItem('foodIds', JSON.stringify(foodIdData.foodIds));
...
function displayData2() {
    ...
    const items = JSON.parse(localStorage.getItem('submarines'));
    console.log('Items to display:', items);
    container.innerHTML = '';

    items.forEach((item, index) => {
        if (item.type === "submarines") {
